<template>
    <div class="layout-padding">
        <el-form inline :model="query" v-auth="'system:role:search'">
            ...
            <el-form-item>
                <el-button  @click="handleQuery()"  type="primary" :loading="loading">查询</el-button>
                <el-button @click="reset" :loading="loading">重置</el-button>
            </el-form-item>
        </el-form>
        <div class="action-box">
              <el-button v-auth="'system:role:add'" @click="handleAdd()" icon="ele-Plus" type="primary">新增角色</el-button>
        </div>

        <el-table v-auth="'system:role:search'"
            class="w100"
            ref="tableListRef"
            v-loading="loading"
            :data="tableList"
            ...
            <el-table-column fixed="right" align="center" label="操作" width="260">
                <template #default="{row}">
                    <el-button v-auth="'system:role:menu'" @click.stop="handlePermission(row)" icon="ele-Check" type="primary" link>分配权限</el-button>
                    <el-button v-auth="'system:role:edit'" @click.stop="handleEdit(row)" icon="ele-Edit" type="warning" link>修改</el-button>
                    <el-popconfirm @confirm="handleDelete(row.id)" width="auto" :title="`确定永久删除【${row.roleName}】吗？`">
                        <template #reference>
                            <el-button v-auth="'system:role:delete'" icon="ele-Delete" type="danger" link>删除</el-button>
                        </template>
                    </el-popconfirm>
                </template>
            </el-table-column>
