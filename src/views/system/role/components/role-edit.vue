<template>
    <el-dialog :title="`${title}角色`" v-model="visible" 
        ...
        <el-form v-loading="loading" ref="formRef" :model="formData" label-width="85px" label-position="right" status-icon label-suffix=":">
            ...
            <el-form-item label="角色编码" prop="roleCode" :rules="{required:true,pattern: /^[a-zA-Z0-9_]{2,30}$/, message: '只允许输入2~30位英文/数字/下划线', trigger: 'blur'}">
                <el-input v-model="formData.roleCode" placeholder="请输入角色编码" maxlength="30" show-word-limit/>
            </el-form-item>
            <el-form-item label="角色状态" prop="status" :rules="{required: true, message: '角色状态为必选项！', trigger: 'change'}">
