...
async function loadRoleOptions() {
    try {
        ...
        if (data && data.length > 0) {
            // 转换为下拉框的目标类型数据
            roleOptions.value = data.map((item:any) => ({value: item.id, label: item.roleName}));
        } else {
            roleOptions.value = [];
        }
    } catch (error) {
...
<template>
    <el-dialog :title="`${title}用户`" v-model="visible" 
        ...
        <el-form v-loading="loading" ref="formRef" :model="formData" label-width="85px" label-position="right" status-icon label-suffix=":">
            <el-row>
                <el-col :span="12">
                    <el-form-item label="用户昵称" prop="nickName" :rules="{required: true, message: '用户昵称为必填项！', trigger: 'blur'}">
                        <el-input v-model="formData.nickName" placeholder="请输入用户昵称" maxlength="30" show-word-limit clearable/>
                    </el-form-item>
                </el-col>
                <el-col :span="12">
                    <el-form-item label="用户账号" prop="username" :rules="{required: true, validator: usernameValidator, trigger: 'blur'}">
                        <el-input v-model.trim="formData.username" placeholder="请输入用户账号" maxlength="30" show-word-limit clearable/>
                    </el-form-item>
                </el-col>
                <el-col :span="12">
                    <el-form-item label="手机号码" prop="mobile" :rules="{validator: mobileValidator, trigger: 'blur'}">
                        <el-input v-model.trim="formData.mobile" placeholder="请输入手机号码" maxlength="20" show-word-limit clearable/>
                    </el-form-item>
                </el-col>
                <el-col :span="12">
                    <el-form-item label="登录密码" prop="password" :rules="{required: true, pattern: /^[a-zA-Z0-9]{6,30}$/, message: '密码必须是6~30数字/字母/下划线！', trigger: 'blur'}">
                        <el-input v-model.trim="formData.password" :disabled="disabled" placeholder="请输入6~30位登录密码" type="password" maxlength="30" show-word-limit clearable/>
                    </el-form-item>
                </el-col>
                <el-col :span="12">
                ...
                <el-col :span="12">
                    <el-form-item label="帐号锁定" prop="accountNonLocked" :rules="{required: true, message: '帐号锁定为必选项！', trigger: 'change'}">
                ...
                <el-col :span="12">
                    <el-form-item label="帐号过期" prop="accountNonExpired" :rules="{required: true, message: '帐号过期为必选项！', trigger: 'change'}">
                ...
                <el-col :span="12">
                    <el-form-item label="密码过期" prop="credentialsNonExpired" :rules="{required: true, message: '密码过期为必选项！', trigger: 'change'}">
                ...
                <el-col :span="12">
                    <el-form-item label="备注" prop="remark" >
                        <el-input v-model="formData.remark" type="textarea" placeholder="请输入备注信息" maxlength="100" show-word-limit/>
                    </el-form-item>   
                </el-col>
            </el-row>
        </el-form>
