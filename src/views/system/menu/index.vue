    ...
    <div class="layout-padding">
        <el-form inline :model="query" v-auth="'system:menu:search'">
            ...
            <el-form-item>
                ...
                <el-button v-auth="'system:menu:add'" @click="handleAdd()" icon="ele-Plus" type="success">新增菜单</el-button>
            </el-form-item>
        </el-form>

        <!-- 树状：data接收的数组元素中要有children属性，必须指定row-key值：数组元素的对象唯一属性名 -->
        <el-table v-auth="'system:menu:search'"
            class="w100"
            ref="tableListRef"
            v-loading="loading"
            :data="tableList"
            ...
            @row-click="toggleRow"
            >
            <el-table-column prop="meta.title" label="菜单名称" align="left" header-align="center" >
                <template #default="{row}">
                    <SvgIcon :name="row.meta?.icon || ''"/>
                    <span class="ml10">{{ row.meta?.title }}</span>
                </template>
            </el-table-column>
            <el-table-column prop="path" label="路由地址" align="left" header-align="center" show-overflow-tooltip/>
            <el-table-column prop="component" label="组件路径" align="left" header-align="center" show-overflow-tooltip/>
            <el-table-column prop="code" label="权限标识" align="left" header-align="center" show-overflow-tooltip/>
            ...
            <el-table-column label="操作" align="center" width="240">
                <template #default="{row}">
                    <el-button v-auth="'system:child:add'" @click.stop="handleAdd(row.id)" v-if="row.type != 2" icon="ele-Plus" type="primary" link>新增下级</el-button>
                    <el-button v-auth="'system:menu:edit'" @click.stop="handleEdit(row)" icon="ele-Edit" type="warning" link>修改</el-button>
                    <el-popconfirm @confirm="handleDelete(row.id)" width="auto" :title="`确定永久删除【${row.meta?.title}】吗？`">
                        <template #reference>
                            <el-button v-auth="'system:menu:delete'" @click.stop icon="ele-Delete" type="danger" link>删除</el-button>
                        </template>
                    </el-popconfirm>
                </template>
            </el-table-column>
