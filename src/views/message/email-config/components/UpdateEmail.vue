  ...
  >
    ...
    >
      <el-form-item label="邮箱标题" prop="emailTitle">
        <el-input v-model="addForm.emailTitle" placeholder="请输入邮箱标题" />
      </el-form-item>
      <el-form-item label="发件邮箱" prop="emailName">
        <el-input v-model="addForm.emailName" placeholder="请输入发件邮箱" />
      </el-form-item>
    ...
    addFormRef.value.validate(async(valid:boolean)=>{
     if(valid){
        try{
         ...
         if(props.dialogTitle=='创建配置邮箱'){
             const {code}=await addEmailConfig(addForm.value)
            if(code==200){
           ...
           const {code}=await updateEmailConfig(addForm.value)
              if(code==200){
